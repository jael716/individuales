...
class Ninja(Persona):
    ...
    def guardar(self):
        # Guardar en CSV
        with open(f'{self.nombre}.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([self.nombre, self.aldea, self.rango])
    
    def cargar(nombre):
        # Cargar desde CSV
        with open(f'{nombre}.csv', 'r') as file:
            reader = csv.reader(file)
            data = next(reader)
            nombre, aldea, rango = data
            return Ninja(nombre, aldea, rango)
...
# Crear y guardar 10 ninjas
for i in range(1, 11):
    ninja = Ninja(f"Ninja{i}", "Konoha", "Genin")
    ninja.guardar()

# Crear y guardar 10 traidores
for i in range(1, 11):
    traidor = Traidor(f"Traidor{i}", "Konoha", True)
    traidor.guardar()
